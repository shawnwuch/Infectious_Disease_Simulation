...
int main() {
	...
	myfile << "transfer_probability    inoculation_fraction    avg_steps    avg_stable_ppl\n";
	for ( int inoculation_fraction = 0; inoculation_fraction <= 95; inoculation_fraction += 5 ) {	
		for ( int transfer_probability = 0; transfer_probability <= 100; transfer_probability += 5 ) {
			...
			myfile << transfer_probability * 0.01 << " " << inoculation_fraction * 0.01 << " " << number_of_steps / number_of_runs \
				   << " " << number_of_stable_people / number_of_runs << "\n";
		}
	}
	myfile.close();
	return 0;
